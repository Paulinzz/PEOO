# Q3 (10pts) – Conserte o erro na movimentação da cobrinha. No seu projeto, 
# caso o usuário direcione a cobrinha para o sentido oposto aonde ela está indo 
# (ex.: a cobrinha está andando para a direita e o usuário aperta "esquerda"), ela volta e se suicida.
#  Isso é um erro, pois no jogo original a cobrinha ignora tal comando e segue andando na mesma direção
#  que já estava. Conserte esse erro no seu código.
...
def gerar_comida():
    comida_x = round(random.randrange(0, (largura- 100) - tamanho_quadrado) / float(tamanho_quadrado)) * float(tamanho_quadrado)
    comida_y = round(random.randrange(0, (altura-100) - tamanho_quadrado) / float(tamanho_quadrado)) * float(tamanho_quadrado)
